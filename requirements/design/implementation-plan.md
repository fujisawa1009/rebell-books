...
publishing.md §7-3 / T-223 の申し送り事項。CEO確認（2026-08-18）により、**フェーズ1より先に完了させる優先フェーズ**に変更。
Cloudflare未接続でも配線・動作確認の大部分は進められる（最終の公開確認のみフェーズ1完了を待つ）。

**方式決定（CEO 2026-08-20）**: 独立cronは新設せず、**既存の夜間自律ループ（T-280ジョブ基盤／`lib/task-queue.js` +
`lib/research-runner.js`）に「記事生成」workTypeとして統合**する。理由: 安全弁（`PAUSED`即停止・1晩あたりの件数上限・
🤖タグによる対外操作の除外）を作り直さずにそのまま使える。

| # | 内容 | 担当 | 状態 | 備考 |
|---|---|---|---|---|
| 2-1 | `.company/jobs/registry.yml` に `rebell-books-article`（仮）ジョブを追加。engine は既存の research-runner を拡張する形にする | AI | 🟡 | T-280の台帳フォーマットに準拠。1晩の生成上限（例: 1〜2本/晩）を明記し、Cloudflare Pages Free の500ビルド/月枠（README §3の注意書き）と整合させる |
| 2-2 | `lib/task-queue.js` の workType 種別に `book-article`（仮称）を追加。既存 `allowedWorkTypes`（`.company/secretary/autonomy-scope.json`）に含めるかはCEO GOが前提（T-180と同じ安全弁方式を踏襲） | AI（実装）/ CEO（GO） | 🟡 | フェーズ3-1「完全自動push」を機能させるには implementation workType 相当の解禁が必要。既存 T-180 の段階解放パターンに合わせ、まず1本を承認ありで試走→問題なければ解禁 |
| 2-3 | research-runner から「`projects/rebell-books` ディレクトリで `scripts/publish.sh` を叩く」導線を1本追加 | AI | ⛔ | 2-1/2-2が前提。既存の `scripts/publish.sh`（commit & push、rebell-books独立git・ai-git.sh不使用）をそのまま呼ぶだけで済む設計 |
| 2-4 | AIが書いた記事1本を試験的にcommit/pushし、配線が正しく動くか確認（Cloudflare未接続のため公開確認はできない） | AI | ⛔ | 2-3完了後に着手可。**公開まで含めた一気通貫確認はフェーズ1完了後に別途実施** |

**DoD**: 夜間自律ループが `content/posts/*.md` を書いて `scripts/publish.sh` を叩くところまで、人の手を介さず到達する（公開の最終確認はフェーズ1完了後）。

---

## フェーズ2.5: 選書パイプライン＋記事化スキルの実装（新規・フェーズ3の前提）
...
| # | 内容 | 担当 | 状態 | 備考 |
|---|---|---|---|---|
| 2.5-1 | 楽天ブックスAPI アプリID発行（審査なし・5分） | **CEO** | 🟡 | 🔴**現時点で未発行（`.env`に`RAKUTEN`系キーなし・2026-08-20確認）。ここが選書パイプライン全体のブロッカー**。sources.mdの軸A（キーワード）・軸B（売れ筋）双方がこのAPIに依存する |
| 2.5-2 | 選書スクリプト実装（軸A キーワード×軸B 売れ筋、`sort=sales`で候補プールを取得） | AI | ⛔ | 2.5-1完了後。データ源は楽天ブックス書籍検索APIのみ（Amazon PA-APIは廃止済み・使用不可） |
| 2.5-3 | 実在確認（openBD、認証不要）でISBN突合。取れない候補は破棄 | AI | 🟡 | 既にbooklist-2026-07.mdで実証済みの手順を流用 |
| 2.5-4 | 記事化スキル `skills/write-book-review.md`（名称は暫定。実体は「紹介記事」生成）を新規作成（T-124） | AI | ✅ | 本タスクで作成（下記参照） |
| 2.5-5 | 生成記事が「一人称の読了・感想表現」を含んでいないかの機械チェック（禁止語リスト）をpublish前に挟む | AI | 🟡 | README §「守るルール」4の担保。スキル内プロンプトだけでなく機械的な後段チェックも入れる（LLMの逸脱を構造的に防ぐ） |

**DoD**: 楽天アプリID発行済み・選書→実在確認→記事生成→禁止語チェックまでが人手なしで1本通る。
...
| # | 内容 | 担当 | 状態 | 備考 |
|---|---|---|---|---|
| 3-1 | 無料紹介記事を10〜15本投稿 | AI（完全自動・CEO確認なし） | 🟡 | **CEO確認済み（2026-08-18・2026-08-20再確認）**: 公開判断もAIに一任。承認キューを通さず自動push＋**事後報告のみ**（朝ダイジェストに公開記事一覧を毎回掲載。CLAUDE.mdの外部アクション承認原則に対する本プロジェクト限定の明示的例外）。フェーズ2.5完了後に着手可。フェーズ1（Cloudflare接続）完了前は「GitHubへのpushまで」を進め、実際の公開確認はフェーズ1完了後に行う。Amazonアソシエイト審査に「オリジナル記事10本以上＋運営者情報」が必須 |
...
CEO確認（2026-08-18／2026-08-20更新）により、**フェーズ2→2.5（worker連携・選書・記事化スキル・AI先行）を先行させる順序**。

```
フェーズ0(完了) → フェーズ2(worker連携・AI先行) → フェーズ2.5(選書パイプライン＋記事化スキル)
                                                    → フェーズ1(CEO:Cloudflare接続等・楽天アプリID発行)
                                                    → フェーズ3(記事投稿・完全自動push＋事後報告のみ)
                                                          → フェーズ1完了で「公開」が確定
...
- フェーズ3（記事投稿）はフェーズ2.5完了後すぐ着手可能（AI完全自動・**事前承認なし・公開後に朝ダイジェストで事後報告**）。ただし実際の「公開」確認はフェーズ1完了が前提。
