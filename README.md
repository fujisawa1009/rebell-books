...
keyPoints:                           # 任意。要点3〜6個（サイト表示には使わない）
  - "..."                            # 将来の別媒体展開（例: 要約スライド化→TikTok動画化）向けの構造化データ
coverImage: "https://..."            # 任意。権利者公式の書影URLのみ（例: openBDのsummary.cover）。
                                      # ホットリンクで参照し複製しない。自前スクリーンショットは著作権上禁止。
                                      # openBDのcover提供率は低く、省略される記事の方が多い想定
coverImageCredit: "openBD"           # 任意。coverImageを付ける場合はセットで書く（出典表記）
---
```

### 記事の見た目を良くするルール（2026-08-20・CEO決定）

デフォルトの4見出し構成に加えて、次を**全ジャンル共通の固定テンプレート**として使う
（詳細な手順は `skills/write-book-review.md` 側にある。ここは概要のみ）:

- **目次・構成の視覚化ブロック**: 本の目次や章立てをコードブロック（ツリー/箱型のテキスト図）で
  見せる。**本文の逐語引用ではなく、公開されている目次情報を独自にレイアウトしたもの**に限る
- **各見出しに1回、要点をblockquoteで抜き出す**（読み飛ばしてもポイントが分かるように）
- **太字は1段落あたり最大2箇所**（多用すると強調の意味が薄れる）
- **書影**: openBD等の公式画像があれば掲載（出典表記つき）。無い場合（openBDのcover提供率は
  低くこちらが多数）は、**「目次・構成の視覚化ブロック」が図解を兼ねる**（ラスター画像を新規に
  生成する仕組みは持たない＝現状のツール構成に画像生成が無いための簡易実装。将来、画像生成を
  追加する場合はここを差し替える）
...
4. **課金していないサービス（ASP案件・AIサブスク等）は使っていないものを推さない。**
   本の記事は「精読した書評」ではなく **公開情報（目次・出版社紹介文・レビュー等）に基づく紹介記事**と位置づける
   （CEO決定 2026-08-20・T-222）。よって「読んだ」前提の縛りは外すが、代わりに次を必ず守る：
   - **一人称の読了・感想表現を使わない**（「読んで感じた」「実際に読むと」等は書かない。禁止語チェックは記事化スキル側で行う）
   - 内容は **openBD等で実在確認した公開情報のみ**を根拠にする（ハルシネーション厳禁）
   - 読者に「精読した書評」だと誤解させる書きぶりにしない（景表法・信頼性の観点。開示文は付けない代わりに書きぶりで担保する）
